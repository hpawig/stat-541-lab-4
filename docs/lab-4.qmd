...
full_df_2018 <- read_excel(path = here ("data",
                                        "wgm2018-dataset-crosstabs-all-countries.xlsx"),
                              sheet = "Full dataset",
                              skip = 0,
                              col_names = TRUE)
...
# Read in 2020 file
wgm_full_df <- read_csv(here("data",
                               "wgm_full_wave2_public_file_final (1)_csv.csv"))

```

```{r}
#| label: wgm2018-create-continents
#| context: setup
...
checkboxInput('continents', 'Display Continents')
...
selectInput(inputId = 'question_of_interest', label = 'What Global Opinion Do You Want To See?',
...
The first visualization is a remake of the original Wellcome Global Monitor 2018 report, (link may be found in the Graph tab). The map tab contains an *interactive* map of the percentage of people that responded "Yes" to questions regarding trust, interest, or belief in topics related to health and science. This map was created using the `leaflet` package and the Natural Earth data set and takes inspiration from the 2018 report's Chart 2.14 (Wellcome Global Monitor 2019: Understanding and Interest in Science and Health, page 39).
...
output$boxplot <- renderPlot({
  ...
  # Checkbox option 1
  # filter out countries with no region
  if (input$continents) { # user checks continents box
      plot_df <- plot_df |> 
  ...
  # Checkbox 2: add continent sizes to plot
  if (input$group_size) {
